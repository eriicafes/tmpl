...
// SetLayoutRoot sets the directory to start searching for layout files.
func (t *templatesParser) SetLayoutRoot(dir string) *templatesParser {
	t.layoutRoot = dir
	return t
}
...
func (t *templatesParser) Autoload(dirs ...string) *templatesParser {
	...
	_, err := parseFiles(t.fsys, autoloadTmpl, t.ext, autoloadFilenames)
	if err != nil {
		t.loadErr = err
		return t
	}
...
// The template name is the last template filename.
//
// If a layout root is not provided only layout filenames from dir and it's sub directories will be matched.
...
func (t *templatesParser) LoadWithLayouts(dir string) *templatesParser {
	...
	groups := walkFilesWithLayout(t.fsys, t.ext, t.layoutRoot, t.layoutFilename, dir)
...
func (t *templatesParser) load(name string, files []string) error {
	...
	tmpl, err := parseFiles(t.fsys, tmpl, t.ext, t.autoload)
	if err != nil {
		return err
	}
	tmpl, err = parseFiles(t.fsys, tmpl, t.ext, files)
	if err != nil {
		return err
	}
